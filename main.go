...
func main() {
	...
	var sections process.Multistring
	...
	for _, f := range flag.Args() {
		if checkVCSRoot {
			root, err := process.VcsRootImportPath(f)
			if err != nil {
				fmt.Fprintf(os.Stderr, "error determining VCS root for file %q: %s", f, err)
				continue
			} else {
				sections = process.Multistring{root}
			}
		}
		// check if input argument is a directory
		info, err := os.Stat(f)
		if err == nil && info.IsDir() {
			err := filepath.Walk(f, process.Visit(writeFile, listDiffFiles, sections, compiledPattern))
			if err != nil {
				log.Println(err)
			}
			continue
